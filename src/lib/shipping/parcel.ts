 ...
 *   times the grams per shelf of its shelf size.
 *
...
export type PackingRules = {
  ...
  /** Height one flat-packed shelf adds to a rack's box. */
  shelfStackCm: number;
};
...
/** One flat-packed rack. */
export function rackBox(p: RackPacking, shelfStackCm: number): { grams: number; box: BoxCm } {
  return {
    grams: p.shelves * p.gramsPerShelf,
    box: {
      length: Math.max(p.heightFt, p.lengthFt) * CM_PER_FT,
      width: p.depthFt * CM_PER_FT,
      height: p.shelves * shelfStackCm,
    },
  };
}
...
export function parcelGrams(lines: readonly ParcelLine[], rules: PackingRules): number | null {
  ...
  for (const l of lines) {
    switch (l.kind) {
      ...
      case "rack": {
        if (!l.packing) return null;
        const r = rackBox(l.packing, rules.shelfStackCm);
