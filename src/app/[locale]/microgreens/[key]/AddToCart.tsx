...
import { setVarietyQuantity } from "../../cart/actions";
 ...
 * hunting for a line of text. It is the only filled panel in this column,
 ...
 *
 * The stepper is deliberately not a free-text number field. Greens sell in
 * whole 100 g units, so every value in between is invalid, and a text input
 * invites "2.5" and "100" and then has to reject them. Two buttons and a
 * readout cannot express a wrong quantity.
 *
 ...
 * **sets** rather than adds (`setVarietyQuantity`), because "add 3" when the
 ...
 * appeared.
 *
 * All validation is repeated server-side in `setVarietyQuantity`. The clamping
...
export function AddToCart({
  ...
  contentKey: string;
  max: number;
  /** 100 g units of this variety already in the cart, 0 if none. */
  inCart: number;
  labels: {
    quantity: string;
    decrease: string;
    ...
    updated: string;
    viewCart: string;
    /** The one-off sow/harvest timing, which belongs with the CTA rather than
     *  loose in the page — it is a condition of the purchase. */
    note: string;
  ...
  /* Seeded from the cart so this control and the header badge agree on load.
     Falls back to 1 for a variety not in the cart — a stepper offering 0 has
     nothing to commit. */
  const [units, setUnits] = useState(() => (inCart > 0 ? inCart : 1));
  const [state, action, pending] = useActionState<FormState, FormData>(
    setVarietyQuantity,
    IDLE,
  );

  /* The label follows the cart, not the local stepper: while this variety is in
